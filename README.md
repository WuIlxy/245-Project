...
**Data Integration** — The two sources are joined on drug/intervention name using string normalization and fuzzy matching (token-sort ratio ≥ 85 via rapidfuzz). Trials with no FDA match are retained with a `has_fda_record = False` flag and null-filled FDA features. Final dataset: **53,628 trials × 150 features**.
